  ...
  for (dirName in lvl2Dirs) {
    ...
    if (fileType == "README"){
      if (length(readmeFileName) > 1){
        ...
        dirMsgs <- c(dirMsgs, checkEncoding(readmeFilePath, dirName, fileType))
      }
    }
    ...
    if (fileType == "DICT"){
      if (length(dictFileName) > 1){
        ...
        dirMsgs <- c(dirMsgs, checkEncoding(dictFilePath, dirName, fileType))
      }
    }
    ...
    if (fileType == "DATA" || fileType == "DICT"){
      if (length(dataFileName) > 1){
        ...
        dirMsgs <- c(dirMsgs, checkEncoding(dataFilePath, dirName, fileType))
      }
    }
    ...
    if (fileType == "MISSING" || fileType == "DATA"){
      if (length(missingFileName) > 1){
        ...
        dirMsgs <- c(dirMsgs, checkEncoding(missingFilePath, dirName, fileType))
      }
    }
