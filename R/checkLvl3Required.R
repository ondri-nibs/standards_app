    ...
    if (!dir.exists(lvl3DirPath)){
      ...
      if (fileType == "FILELIST"){
        if (length(filelistFileName) > 1){
          ...
          dirMsgs <- c(dirMsgs, checkForRaggedRows(filelistFilePath, lvl3DirName, fileType))
        }
      }
      ...
      if (fileType == "MISSING"){
        if (length(missingFileName) > 1){
          ...
          dirMsgs <- c(dirMsgs, checkForRaggedRows(missingFilePath, lvl3DirName, fileType))
        }
      }
      
      
      # DICT file. Cannot check DICT file as it is a text file.
      if (fileType == "DICT"){
