...
checkLvl1Required <- function(lvl1DirPath, fileType, lvl1RequiredFileTypes = NULL){
  ...
  if (fileType == "README"){
    ...
    if (length(readmeFileName) > 1){
      ...
      flaggedMsgs <- c(flaggedMsgs, checkForRaggedRows(readmeFilePath, dirName, fileType))
    }
  }
  ...
  if (fileType == "DICT"){
    if (length(dictFileName) > 1){
      ...
      flaggedMsgs <- c(flaggedMsgs, checkForRaggedRows(dictFilePath, dirName, fileType))
    }
  }
  ...
  if (fileType == "DATA" || fileType == "DICT"){
    if (length(dataFileName) > 1){
      ...
      flaggedMsgs <- c(flaggedMsgs, checkForRaggedRows(dataFilePath, dirName, "DATA"))
    }
  }
  ...
  if (fileType == "MISSING" || fileType == "DATA"){
    if (length(missingFileName) > 1){
      ...
      flaggedMsgs <- c(flaggedMsgs, checkForRaggedRows(missingFilePath, dirName, "MISSING"))
    }
  }
